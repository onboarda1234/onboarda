...
| CA-7 | ~~Verify API-user role needs **"Rescreen on demand"**~~ — **RESOLVED: not needed** | ☑ | P2 | — | **Code-confirmed (2026-07-04):** `monitoring_automation.py` has *no screening-provider imports*; monitoring status is decoupled from screening (derives from stored data, not a re-screen), and re-screening an existing customer would hit the duplicate-external-id→ERRORED path anyway. RegMind does **not** trigger programmatic CA rescreens → no 403. **Decision A: do NOT grant in prod** unless/until an explicit "re-screen now" feature is built. |
...
| PROD-0b | Confirm/create a **production CA workspace + API credential** (live equiv of `ca-staging-api`) | ◐ | P1 | — | Prod CA workspace **exists** (RegMind Ltd production account, confirmed via account switcher). Config created there (PROD-1). **Still to do:** create the prod API user credential (deferred — decision A: no "Rescreen on demand"; do at PROD-2/pilot) and the prod plan/subscription (decision D: at pilot launch). |
| PROD-1 | Replicate both risk models + screening config to the **production CA workspace** | ☑ | P1 | PROD-0b | **DONE (2026-07-04, prod workspace, verified value-by-value vs sandbox).** Prod UUIDs → **Person model `019f2d20-4ac3-754e-a179-a25f41152eda`** (v1 Active), **Company model `019f2d28-ba87-739e-905d-a3f3afe847f1`** (v1 Active), **Screening config `019f2d2f-14a9-7ee7-bbfb-e617e1b8df89`** (v1 Active), source collection `regmind-default-sources-v1` (nested, name-only). Threshold 70, All jurisdictions, 1/4 collections, screening-only scoring (decision B). **These IDs feed PROD-2.** Open: (a) model↔config association is NOT a console field — PROD-2 must confirm how CA applies Person-vs-Company model (auto by entity type, or via API); (b) prod model names differ from sandbox ("Person v1"/"Company v1" vs `regmind-default-risk-model-*`) — cosmetic, wired by UUID. |
| PROD-2 | Wire prod CA config into prod backend (secrets + env; `WORKSPACE_MODE=production`, prod `SCREENING_CONFIG_ID`, prod CA creds) | ⛔ | P1 | PROD-0, PROD-1 | Prod IDs now available (PROD-1): `SCREENING_CONFIG_ID=019f2d2f-14a9-7ee7-bbfb-e617e1b8df89`; model IDs 019f2d20-… (person) / 019f2d28-… (company). Apply once prod infra + creds exist. **Confirm at wiring:** how CA applies the Person-vs-Company risk model (no console link exists — likely auto-by-entity-type or via the create-and-screen API call); verify sandbox behavior and replicate. |
