...
**Last reconciled:** 2026-07-15 (base `main` = `dd4784b`; RSMP review pack #751, Tier 0A parser/mapping #753, and Tier 0B fail-closed routing #755 merged; #755 AWS-staging-validated on backend `regmind-staging:847` and worker `regmind-verification-worker:295`; Tier 0A activation remains OFF; Tier 0C is outstanding. Prior full reconciliation: 2026-07-11 on `6ba253d`, including P12-1 #738 and DCI-006 #739 staging validation; see [**Re-audit 2026-07-11**](#re-audit-2026-07-11-d23cc45--reconciliation)).
**RSMP status:** Tier 0B is merged and staging validated at `dd4784bdf270cb532c9c290e6f16e826ea2776ba`. Tier 0A remains inactive. Within RSMP, Tier 0C is the final remaining pilot-readiness workstream and is not complete; no recomputation has been executed and no production-readiness claim is made.
...
|-----------|:--:|-------------------------|:--:|
| 🔴 DCI-108 | HIGH | Risk parser **under-scores** "very complex" ownership structure → 3 at `rule_engine.py:1219-1273`; combined with DCI-109 can flip an application **MEDIUM→LOW** (risk understatement). Fix = exact-enum rewrite + recompute. **Pilot-relevant** | 🔨 Tier 0A parser/mapping #753 and Tier 0B routing #755 merged; #755 staging validated at `dd4784b`. Activation remains OFF; Tier 0C activation/recomputation is outstanding. |
| 🔴 DCI-109 | HIGH | "non-regulated" resolves to 1 via **dict-ordering fall-through** at `rule_engine.py:1219-1273` (should score higher); same MEDIUM→LOW flip risk. **Pilot-relevant** | 🔨 Tier 0A parser/mapping #753 and Tier 0B routing #755 merged; #755 staging validated at `dd4784b`. Activation remains OFF; Tier 0C activation/recomputation is outstanding. |
...
BSA-004 general CSRF bypass). For **DCI-108/109**, Tier 0A parser/mapping #753 and Tier 0B
fail-closed routing #755 are merged, and #755 is staging validated at `dd4784b`; activation
remains OFF. Within RSMP, **Tier 0C is the final remaining pilot-readiness workstream** and is
not complete. DCI-110 (MED, over-scoring),
...
**9 (Production readiness, last)**. **2026-07-09/10 batch closed 3 of the 4 code pilot blockers** (#728 item 26, #729 P13-1, #730 P11-2) — only **P12-1** (supervised) remains — and remediated the Applications-page readiness-audit Critical/High (#731→#732) plus role-matrix (#733), clean-approval (#734), and UI action-gate (#735); the audit re-run verdict is **READY FOR PILOT WITH CONTROLS**. **The 2026-07-11 full-consolidated audit re-run (`d23cc45`) walked back the prior "≈94–96% pilot-ready" figure:** it surfaced DCI-108/109 and the BSA-003/004 cluster plus four confirmed partials. Since that re-run, RSMP Tier 0A parser/mapping #753 and Tier 0B fail-closed routing #755 have merged, and #755 is staging validated at `dd4784b` with activation OFF. Within RSMP, Tier 0C is the final remaining pilot-readiness workstream and remains outstanding. Production-readiness ≈ 35–40%
