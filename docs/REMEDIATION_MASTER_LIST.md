...
**Last reconciled:** 2026-07-11 (base `main` = `d23cc45`; full-consolidated audit RE-RUN folded — see [**Re-audit 2026-07-11**](#re-audit-2026-07-11-d23cc45--reconciliation) section below; prior reconcile 2026-07-10 on `84de284` after #735, staging validated `8a0fdef`/`0e1a4ee`).
**2026-07-09/10 batch — 3 of the 4 controlled-pilot CODE blockers CLOSED (merged + AWS-staging-validated):**
**#729 (P13-1, back-office stored-XSS)** PASS-with-limitation · **#728 (item 26 / BSA-002, shared fail-closed rate limiter)** PASS-with-limitation (migration v2.51 `shared_rate_limits`) · **#730 (P11-2, dep-CVE + pip-audit CI gate)** PASS-with-limitation. Only remaining 🔴 code pilot blocker: **P12-1** (supervised). Also landed: **#731→#732** (Applications audit-log isolation by immutable `application_id`, migration v2.50; APP-727-001/002 CLOSED), **#733** (P9-13 role×route matrix harness), **#734** (clean-approval e2e, APP-AUD-003), **#735** (APP-AUD-001 role-UI/authz alignment). Migration sequence v2.47…v2.51 clean, no collision, no up-front-index (#717-class) bug. Staging == latest `origin/main`; residual noise = the known v2.47 DCI-006 off-canon logs (remediation SQL **merged as #739**, `9d597ea`; Codex runs it on staging + reboots to install the 3 constraints). **2026-07-11 audit re-run** additionally surfaced net-new HIGH findings (risk-parser under-scoring DCI-108/109; supervisor-route/CSRF cluster BSA-001–004 re-run) + 4 confirmed partials — see the [Re-audit 2026-07-11](#re-audit-2026-07-11-d23cc45--reconciliation) section.
...
> 🔴 **Controlled-pilot blockers (code).** **3 of 4 CLOSED (merged + staging-validated 2026-07-09/10); only P12-1 remains:**
> - 🔴 **P12-1** — Regulated-record deletion protection (DCI-001/003, CRITICAL) — Phase 11 — ⬜ **OPEN (supervised)**
...
| APP-727-audit-writer-id-1 | Med | **NEW residual** — populate `application_id` (and `request_id`) in ALL audit writers, decision/sign-off first, to fully complete APP-727-001 immutable-id isolation (those rows are still ref-only today) | — | ⬜ pending |
...
| P12-5 | Status-column CHECK constraints | DCI-006 | MED | Canon-constant CHECK constraints for all 8 status/enum columns (Migration v2.47, steady-state no-op boots via constraint-def comparison; fail-closed `clients.status` backfill; SQLite→PG migrator per-row SAVEPOINTs). Bonus repair: `Severity.WARNING` added to supervisor enum — 6 call sites crashed with AttributeError before their audit INSERT. 20 tests; live-PG probe 19-green | [#716](https://github.com/onboarda1234/onboarda/pull/716) | ✅ merged + deployed. **Residual:** on staging the 3 constraints (clients.status, agent_executions.status/source) still SKIP because of 4 legacy off-canon rows (`disabled`/`direct_probe`/`fixture`/`staging_direct_probe`) — remediation SQL `scripts/dci006_staging_remediation.sql` is **merged as [#739](https://github.com/onboarda1234/onboarda/pull/739)** (`9d597ea`; Codex runs it on staging, then reboots so v2.47 installs the 3 constraints) — the source of the recurring 6 DCI-006 CloudWatch ERROR logs. Re-audit **DCI-104** (re-run) reconfirmed all 3 constraints ABSENT + flagged **54 unindexed FKs** (follow-up) |
...
| 🔴 BSA-001 (re-run) | HIGH | Supervisor routes subclass `tornado.web.RequestHandler` directly (`supervisor/api.py:200`, registration `:644-660`) — bypassing BaseHandler auth/CSRF/rate-limit/security-header middleware. Consolidate onto BaseHandler | ⬜ scoped |
| 🔴 BSA-002 (re-run) | HIGH | Supervisor actor is **client-forgeable** (`supervisor/api.py:301,381-405,445-459`) — server must derive the actor from the authenticated session, never trust the request body | ⬜ scoped |
| 🔴 BSA-003 (re-run) | HIGH | Supervisor reviews/overrides/escalations persist to a **local SQLite `arie.db`** (`human_review.py:54-115,392-402,430-481`); `DB_PATH` unset on staging → writes land on ephemeral container disk and errors are **swallowed, returning 200**. Fail-closed PostgreSQL persistence required. **Pilot-relevant** (audit-record loss) | ⬜ scoped |
| 🔴 BSA-004 (re-run) | HIGH | **General CSRF bypass**: `if "/webhook" in self.request.uri: return` (`base_handler.py:515-554`) is a substring match — ANY path containing `/webhook` skips CSRF. Replace with an exact-path allowlist. **Pilot-relevant** | ⬜ scoped |
...
| DCI-104 (re-run) | P12-5 / #716 + DCI-006 | All 3 v2.47 CHECK constraints still **ABSENT** on staging **+ 54 unindexed FKs**. *(Correction: the DCI-006 remediation SQL was NOT "on main" — it landed via **[#739](https://github.com/onboarda1234/onboarda/pull/739)**, now MERGED `9d597ea`.)* | ✅ #739 merged (SQL) · ⬜ staging run + reboot (Codex) · ⬜ 54-FK-index follow-up |

**Net effect on readiness:** the earlier "≈94–96% pilot-ready" estimate is **walked back**.
DCI-108/109 (risk understatement, can flip MEDIUM→LOW) and BSA-003/004 (supervisor
audit-record loss + general CSRF bypass) are pilot-relevant and were not in that count.
Retriage pending founder decision; the DCI-108/109 risk-parser fix is offered as the next
code PR (exact-enum rewrite + recompute).
