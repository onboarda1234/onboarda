...
**Last reconciled:** 2026-07-15 (base `main` = `a823fb6`; RSMP review pack #751, Tier 0A parser/mapping #753, Tier 0B fail-closed routing #755, and PR-1b PEP runtime alignment #764 merged; #764 AWS-staging-validated on backend `regmind-staging:850` and worker `regmind-verification-worker:298`; Tier 0A activation remains OFF; Tier 0C is outstanding. Prior full reconciliation: 2026-07-11 on `6ba253d`, including P12-1 #738 and DCI-006 #739 staging validation; see [**Re-audit 2026-07-11**](#re-audit-2026-07-11-d23cc45--reconciliation)).
**RSMP status:** PR-1b is merged and staging validated at `a823fb6491ea35a1647800853a97dc7a0f328b6f`: every declared PEP role scores 4, with the existing High floor, EDD, and approval-routing behaviour unchanged. Tier 0A remains inactive. Within RSMP, Tier 0C is the next risk-scoring pilot gate and is not complete; no recomputation has been executed and no production-readiness claim is made.
...
|-----------|:--:|-------------------------|:--:|
| 🔴 DCI-108 | HIGH | Risk parser **under-scores** "very complex" ownership structure → 3 at `rule_engine.py:1219-1273`; combined with DCI-109 can flip an application **MEDIUM→LOW** (risk understatement). Fix = exact-enum rewrite + recompute. **Pilot-relevant** | 🔨 Tier 0A parser/mapping #753, Tier 0B routing #755, and PR-1b PEP runtime alignment #764 merged; #764 staging validated at `a823fb6`. Activation remains OFF; Tier 0C activation/recomputation is outstanding. |
| 🔴 DCI-109 | HIGH | "non-regulated" resolves to 1 via **dict-ordering fall-through** at `rule_engine.py:1219-1273` (should score higher); same MEDIUM→LOW flip risk. **Pilot-relevant** | 🔨 Tier 0A parser/mapping #753, Tier 0B routing #755, and PR-1b PEP runtime alignment #764 merged; #764 staging validated at `a823fb6`. Activation remains OFF; Tier 0C activation/recomputation is outstanding. |
...
BSA-004 general CSRF bypass). For **DCI-108/109**, Tier 0A parser/mapping #753, Tier 0B
fail-closed routing #755, and PR-1b PEP runtime alignment #764 are merged; #764 is staging
validated at `a823fb6`, with every declared PEP role scoring 4 and the existing High floor, EDD,
and approval routing unchanged. Activation remains OFF. Within RSMP, **Tier 0C is the next
risk-scoring pilot gate** and is not complete. DCI-110 (MED, over-scoring),
...
**9 (Production readiness, last)**. **2026-07-09/10 batch closed 3 of the 4 code pilot blockers** (#728 item 26, #729 P13-1, #730 P11-2) — only **P12-1** (supervised) remains — and remediated the Applications-page readiness-audit Critical/High (#731→#732) plus role-matrix (#733), clean-approval (#734), and UI action-gate (#735); the audit re-run verdict is **READY FOR PILOT WITH CONTROLS**. **The 2026-07-11 full-consolidated audit re-run (`d23cc45`) walked back the prior "≈94–96% pilot-ready" figure:** it surfaced DCI-108/109 and the BSA-003/004 cluster plus four confirmed partials. Since that re-run, RSMP Tier 0A parser/mapping #753, Tier 0B fail-closed routing #755, and PR-1b PEP runtime alignment #764 have merged; #764 is staging validated at `a823fb6`, every declared PEP role scores 4 with existing High-floor/EDD/approval behaviour unchanged, and activation remains OFF. Within RSMP, Tier 0C is the next risk-scoring pilot gate and remains outstanding. Production-readiness ≈ 35–40%
