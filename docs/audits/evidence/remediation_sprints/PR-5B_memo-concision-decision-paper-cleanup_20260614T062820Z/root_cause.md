...
## Second Corrective Root Cause: Stale Generated Memo Reuse

After PR #483 was merged and deployed to staging, `/api/version` correctly
reported merged main SHA `7cf095eeeb619b95fbe08764da529f00c7225b94`, but
focused API/PDF smoke still returned memo `357` with
`memo_build_git_sha=4e2262dc14db86a6e3caacb617182fbe8579ae5c`.

The memo endpoint reused the existing memo because its source-input
`raw_output_hash` matched. The idempotency fingerprint only represented input
data and did not include the memo output profile/generation contract. As a
result, a memo generated by the older PR-5B output profile could continue to be
served even after the code that fixes risk wording, blocker summarisation, and
validation copy had been deployed.

Second corrective fix strategy:

- Define one `MEMO_OUTPUT_PROFILE_VERSION` constant in `memo_handler.py`.
- Store that version in generated memo metadata as
  `metadata.memo_output_profile.profile_version`.
- Include `MEMO_OUTPUT_PROFILE_VERSION` in the server-side memo input
  fingerprint payload and advance the fingerprint prefix to `memo-input-v2`.
- Reject idempotent reuse when stored memo metadata is missing the current
  output profile version, even if the old source-input hash matches.
- Preserve idempotent reuse for memos generated by the current output profile.
