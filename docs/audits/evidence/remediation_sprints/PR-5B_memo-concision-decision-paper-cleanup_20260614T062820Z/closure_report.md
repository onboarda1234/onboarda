...
Second corrective re-diagnosis after PR #483 merge/deploy confirmed on staging
SHA `7cf095eeeb619b95fbe08764da529f00c7225b94`:

- `/api/version` and PDF generation were healthy.
...
- The reused memo still contained `HIGH risk with score 25/100`.
- PR-5B remained incomplete because fixed code was deployed but stale generated
  memo output was still eligible for idempotent reuse.
...
Second corrective root cause:

- The memo idempotency fingerprint represented source inputs only.
- It did not include the memo output profile/generation contract.
- Stored memos with matching input hashes could be reused even if they were
  generated by an older memo output profile that contained contradictory
  narrative text.
...
- Second corrective branch versions the memo output profile in metadata and
  idempotency fingerprinting.
- Second corrective branch rejects reuse of old-profile or missing-profile memo
  rows even when source inputs are unchanged.
...
- Updated memo integrity tests for output-profile-aware fingerprinting and stale
  stored-output rejection.
...
5298 passed, 17 skipped in 190.80s. Second corrective memo-output cache
invalidation patch full relevant backend suite passed: 5301 passed, 17 skipped
in 233.50s. See `full_suite_results.md`.

GitHub CI passed for PR #482 before the initial merge and for PR #483 before
the first corrective merge. GitHub CI must also pass for the second corrective
memo-output cache invalidation PR before merge.
...
local memo/PDF smoke also passed. PR #483 merged-main staging API/PDF smoke
failed because stale old-profile memo output was reused. Second corrective
branch local memo/idempotency validation passed. See `api_smoke.md`.
...
- Second corrective staging API/PDF smoke must prove old-profile memo output is
  regenerated and no longer contains `HIGH risk with score 25/100`.
- Corrective staging browser memo view still requires visual proof.
- GitHub CI must pass on the second corrective PR branch.

## Items Not Closed By This PR

- No unrelated remediation item is closed by PR-5B.
- PR-5B is not complete until the second corrective PR has merged-main staging
  validation.

## Final Closure Verdict

PARTIALLY FIXED at second corrective branch stage. Not complete until the second
corrective PR is merged, deployed to staging, `/api/version` matches the
corrective merge SHA, API/PDF smoke passes, browser smoke passes, and the
evidence pack is completed.
