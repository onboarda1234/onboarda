...
- Verify no nested dictionary in application detail returns `approval_ready=true` while `approval_blocking=true`.
- Check current `screening_truth_summary` remains safe.
- Ensure `latest_memo_data.metadata.screening_state_summary` is safe.
