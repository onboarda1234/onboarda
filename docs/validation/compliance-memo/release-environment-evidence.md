# Release validation environment evidence

Initial remediation capture: `2026-08-01T05:53:06Z`

Final-tree gate restart capture: `2026-08-01T06:30:22Z`

Post-review authoritative restart capture: `2026-08-01T07:18:59Z`

| Evidence | Value |
|---|---|
| Execution mode | Remediated local validation host (approved alternative to a clean CI runner) |
| Host identity | Anonymous local validation host `regmind-validation-macos-arm64-01` |
| Operating system | macOS 26.3.1 (25D771280a), arm64 |
| Root filesystem | 228 GiB total; **10.16 GiB available** immediately before the post-review authoritative gate |
| Memory | 8 GiB physical; 40% system-wide free at capture |
| Python | CPython 3.12.13 in `/tmp/regmind-release-venv` |
| Test framework | pytest 9.0.3 |
| PDF renderer | WeasyPrint 69.0 |
| PostgreSQL client/server | PostgreSQL 16.14 (Homebrew), SSL `on` |
| Disposable test database | `regmind_release_validation_20260801_r3` (fresh for the post-review authoritative gate) |
| `TEST_POSTGRES_DSN` policy | Localhost database above, `sslmode=require`; password-free local role; DSN contains no secret |
| Branch | `codex/compliance-memo-workspace` |
| Branch HEAD before test | `5090e2c5a62f9e5d1f8a9ca992fbc47ad1b7e1af` |
| Product patch SHA-256 | `092366a3f769b144971be0b3e71f2addf5814b4bb3d42f461388126f1dd97f23` |
| `origin/main` SHA | `c667f95ff8ae892bcc8cafe27efa1151cc7d92f6` |

## Environment remediation

Before remediation, the root filesystem had only 121 MiB available and the first release attempt failed with `Errno 28`.

The restart removed only named disposable caches: Google/Brave render caches, Codex/WhatsApp/Python/updater caches, and npm download caches. Browser profiles, credentials, repository data, Codex bundled runtimes, and Playwright binaries were preserved. Temporary validation directories and the failed-run database were also removed before rebuilding the clean environment.

Available space was verified at 12 GiB after dependencies and the disposable PostgreSQL database were created. No product source was changed to accommodate the infrastructure failure.

GitHub advanced from `aa16a0f` to `c667f95` during the first clean run. The implementation branch was fast-forwarded without conflict, a second fresh PostgreSQL database was created, available space was reverified at 10.14 GiB, and the complete gate was restarted from zero. The earlier green run was not used as the merge gate.

PR review then identified release-hardening gaps in PDF CSS-value sanitisation, rationale-save lock atomicity, PDF preview request identity, the UI generation permission guard, and the screenshot harness. Those findings were corrected without changing the approval handler, lifecycle, database, endpoint paths, or workflow. Because product code changed, the prior 8,909-pass run was discarded as the merge gate. Four explicitly named disposable download/build caches (`pip`, `pnpm`, `node-gyp`, and `ms-playwright-go`) were removed, providing 10.16 GiB free; a third fresh PostgreSQL database was created and the full suite was restarted from test 1.

## Authoritative full automated gate

- Final source tree: branch HEAD `5090e2c5a62f9e5d1f8a9ca992fbc47ad1b7e1af` plus product patch SHA-256 `092366a3f769b144971be0b3e71f2addf5814b4bb3d42f461388126f1dd97f23`.
- Runner identifier: `regmind-validation-macos-arm64-01` / local validation run started `2026-08-01T07:18:59Z`.
- Database policy: PostgreSQL-only tests ran with `TEST_POSTGRES_DSN` against PostgreSQL 16.14 over SSL.
- Collected: 8,924 tests plus one collection-time policy skip.
- Result: **8,910 passed, 11 skipped, 4 expected failures, 0 failed, 0 errors**.
- Duration: 1,145.03 seconds (19m 05s).
- Infrastructure errors: none.
- Failed test names/root causes: none.
- Warnings: 2,272 non-failing warnings. Most are repeated `InsecureKeyLengthWarning` notices caused by the disposable local JWT test key; the remainder are existing datetime/sqlite deprecations. No warning represents a product exception or infrastructure failure.
- Raw output: `full-test-run.log` in this evidence directory.
