...
| **Screening Queue (back office)** | ✅ **VALIDATED — CHANGE-CONTROLLED** | 2026-07-17 | The Screening Queue list surface: `/api/screening/queue` (states, filters, fixture gating, scan/evidence caps, evidence hydration, triage block), queue UI (7-column layout, badges, pagination), queue resolver states, and the governed refresh harness. NOT a pilot-ready claim for the end-to-end screening workflow — the Screening Review page (SRP-3/4, actively developed under founder direction) gates that verdict. Guard tests: `test_screening_queue.py`, `test_screening_queue_state_integrity.py`, `test_seed_screening_qa_fixtures.py`, `test_fixture_exclusion.py`, `test_inline_screening_runtime.py`, `test_backoffice_ca_truthflow_static.py`, `test_srp2_refresh_stale_screening_reports.py`, `test_srp2_batch1_regressions.py`. Evidence: [`docs/compliance/screening_queue_module_card.md`](docs/compliance/screening_queue_module_card.md). |
