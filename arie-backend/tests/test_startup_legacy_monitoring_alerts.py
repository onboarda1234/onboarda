...
def test_sqlite_init_db_repairs_legacy_application_enhanced_requirement_monitoring_columns(tmp_path):
    """Legacy Enhanced Requirement tables are repaired before monitoring indexes are created."""
    env = os.environ.copy()
    env["DATABASE_URL"] = ""
    env["ENVIRONMENT"] = "development"
    env["DB_PATH"] = str(tmp_path / "legacy_requirements.db")
    ...
    _run_startup_script(
        ...
        def legacy_schema_without_monitoring_requirement_columns(schema_sql):
            legacy_sql, replacements = re.subn(
                r"(\\n\\s*linked_document_id TEXT REFERENCES documents\\(id\\) ON DELETE SET NULL,\\n)"
                r"\\s*monitoring_alert_id INTEGER,\\n"
                r"\\s*monitoring_document_id TEXT,\\n"
                r"\\s*due_date TEXT,\\n",
                r"\\1",
                schema_sql,
                count=1,
            ...
            assert replacements == 1
            assert "idx_app_enhanced_req_monitoring_alert" not in legacy_sql
            assert "idx_app_enhanced_req_monitoring_doc" not in legacy_sql
            return legacy_sql

        legacy_db = db_module.get_db()
        legacy_db.executescript(
            legacy_schema_without_monitoring_requirement_columns(db_module._get_sqlite_schema())
        )
        legacy_db.commit()
        pre_cols = {
            r["name"]
            for r in legacy_db.execute("PRAGMA table_info(application_enhanced_requirements)").fetchall()
        }
        assert "monitoring_alert_id" not in pre_cols
        assert "monitoring_document_id" not in pre_cols
        assert "due_date" not in pre_cols
        legacy_db.close()
        ...
        cols = {
            r["name"]
            for r in db.execute("PRAGMA table_info(application_enhanced_requirements)").fetchall()
        }
        indexes = {
            r["name"]
            for r in db.execute("PRAGMA index_list(application_enhanced_requirements)").fetchall()
        }
        db.close()
...
def test_pg_init_db_repairs_legacy_application_enhanced_requirement_monitoring_columns():
    """PostgreSQL startup repairs old requirement tables before creating monitoring indexes."""
    dsn = os.environ.get("TEST_POSTGRES_DSN") or os.environ.get("DATABASE_URL_TEST")
    if not dsn:
        pytest.skip("Set TEST_POSTGRES_DSN or DATABASE_URL_TEST to enable PG startup regression test.")
    ...
    _run_startup_script(
        ...
        legacy_db = db_module.get_db()
        legacy_db.executescript(
            legacy_schema_without_monitoring_requirement_columns(db_module._get_postgres_schema())
        )
        legacy_db.commit()
        pre_cols = {
            r["column_name"]
            for r in legacy_db.execute(
                "SELECT column_name FROM information_schema.columns "
                "WHERE table_name = 'application_enhanced_requirements'"
            ).fetchall()
        }
        assert "monitoring_alert_id" not in pre_cols
        assert "monitoring_document_id" not in pre_cols
        ...
        indexes = {
            ...
            for r in db.execute(
                ...
                "WHERE tablename = 'application_enhanced_requirements'"
            ).fetchall()
        }
        db.close()
        db_module.close_pg_pool()
