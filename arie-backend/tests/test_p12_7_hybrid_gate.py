...
def test_aggregate_surviving_inconclusive_flags_document():
    results = [
    ...
    assert agg["overall"] == "flagged"
    assert any("deterministic resolution failed" in w for w in agg["warnings"])
...
def _captured_ids(stub):
    ...
    return sorted(c.get("id") for c in stub.captured_overrides[-1])
...
def test_flag_wiring_reads_config_at_call_time():
    """The gate must consult config dynamically (monkeypatchable), and the
    legacy selection line must survive untouched for the flag-off path."""
    ...
    assert "ai_hybrid_checks = pure_ai + still_inconclusive" in src
