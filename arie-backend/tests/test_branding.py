...
class TestBrandDictionary:
    ...
    def test_portal_name_default(self):
        from branding import BRAND
        assert BRAND["portal_name"] == "RegMind Portal"

    def test_backoffice_name_default(self):
        from branding import BRAND
        assert BRAND["backoffice_name"] == "RegMind Compliance Workspace"

    def test_platform_name(self):
        from branding import BRAND
        assert BRAND["platform_name"] == "RegMind"
    ...
    def test_pdf_footer_default(self):
        from branding import BRAND
        assert BRAND["pdf_footer"] == "Generated by RegMind"
    ...
    def test_copyright_year(self):
        from branding import BRAND
        assert BRAND["copyright"] == "\u00a9 2026 RegMind. All rights reserved."

    def test_company_legal(self):
        from branding import BRAND
        assert BRAND["company_legal"] == "Your Financial Institution"
...
class TestConvenienceAccessors:
    ...
    def test_portal_name(self):
        ...
        assert portal_name() == "RegMind Portal"

    def test_backoffice_name(self):
        from branding import backoffice_name
        assert backoffice_name() == "RegMind Compliance Workspace"

    def test_powered_by(self):
        from branding import powered_by
        assert powered_by() == "Generated by RegMind"
    ...
    def test_pdf_footer(self):
        ...
        assert pdf_footer() == "Generated by RegMind"
...
class TestNoBrandHardcoding:
    ...
    def test_customer_facing_defaults_use_regmind(self):
        from branding import BRAND
        customer_facing = {
            "portal_name": BRAND["portal_name"],
            "backoffice_name": BRAND["backoffice_name"],
            "platform_name": BRAND["platform_name"],
            "pdf_header": BRAND["pdf_header"],
            "pdf_footer": BRAND["pdf_footer"],
            "copyright": BRAND["copyright"],
            "email_from_name": BRAND["email_from_name"],
        }
        assert all("RegMind" in value for value in customer_facing.values())
        assert all("onboarda" not in value.lower() for value in customer_facing.values())
        assert all("arie" not in value.lower() for value in customer_facing.values())

    def test_email_uses_regmind_domain(self):
        from branding import BRAND
        assert BRAND["support_email"].endswith("@regmind.co")
        assert BRAND["email_from_address"].endswith("@regmind.co")

    def test_website_uses_regmind_domain(self):
        from branding import BRAND
        assert BRAND["website"] == "https://regmind.co"

    def test_legacy_technical_identifiers_remain_stable(self):
        from branding import BRAND
        assert BRAND["cookie_prefix"] == "arie"
        assert BRAND["metric_prefix"] == "arie"
