...
- Whether `GET /v2/entity-screening/risks/{risk_id}` returns the same `CARiskDetail` root shape for all taxonomy families. Current C1 tests synthesize sanctions, watchlist, PEP, RCA, media, and company shapes at `arie-backend/tests/test_screening_complyadvantage_normalizer.py:145-171`.
