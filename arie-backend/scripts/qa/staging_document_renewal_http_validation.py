...
def validate_real_routes(
    *,
    ...
    binding = uploaded.get("binding") or (uploaded.get("request") or {}).get(
        "upload_binding"
    ) or {}
    if (
        status != 201
        or uploaded.get("status") != "upload_received"
        or (uploaded.get("request") or {}).get("request_id") != spec["request_id"]
        or binding.get("renewal_request_id") != spec["request_id"]
        or binding.get("application_id") != spec["application_id"]
        or binding.get("customer_id") != spec["customer_id"]
        or binding.get("person_id") != spec["person_id"]
        or binding.get("original_document_id") != spec["document_id"]
        or binding.get("document_type") != spec["document_type"]
        or binding.get("binding_status") != "bound"
    ):
        raise HttpValidationError("real upload route returned an invalid binding")
